...
use crate::{
    core::{
        choice::{
            NoteActions, NoteApplication, NoteRequirement, SoundAction, SoundActionMode,
            VariableApplications, VariableInput,
        },
        path::Path,
    },
...
use super::util::{build_option, build_vec, confirmed, input_builder};
...
pub fn build_sound_action() -> Result<SoundAction> {
    ...
    let action = SoundAction {
        ...
        seek: answers
            .get("seek")
            .map(|answer| TemplatableValue::value(answer.as_int().unwrap().try_into().unwrap())),
        speed: answers
            .get("speed")
            .map(|answer| TemplatableValue::value(answer.as_float().unwrap())),
    };
...
pub fn build_variable_applications() -> Result<VariableApplications> {
    build_vec("Add another variable?", true, build_variable).map(|vec| vec.into_iter().collect())
}
