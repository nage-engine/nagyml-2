...
use requestty::{Answers, PromptModule, Question, Answer};

use crate::{cmd::builder::{util::{build_option, build_vec}, core::{build_input, build_note_actions, build_variable_applications, build_sound_action, build_path}, text::build_text}, text::{display::Text, templating::{TemplatableString, TemplatableValue}}, core::{choice::Choice, prompt::Prompt}};
...
fn build_choice_response(model: usize) -> Result<(Option<Text>, Option<Answer>)> {
	let result = if model == 0 {
		...
		let tag = build_option(
			"Should a trait tag display next to the response?", false,
			|| {
...
fn build_choice(model: usize) -> Result<Choice> {
	...
	let info_pages: Option<Vec<TemplatableString>> = build_option("Unlock info pages?", false, || {
	...
	let sounds = build_option("Add sound actions?", false, || build_vec("Add another sound action?", false, build_sound_action))?;
...
pub fn build_prompt() -> Result<Prompt> {
	let text_lines = build_option(
		"Should this prompt display text?", true,
