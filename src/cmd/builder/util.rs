use anyhow::Result;
use requestty::{Answers, Question, Answer, question::InputBuilder};
...
pub fn build_vec<T, F>(confirm: &str, newline: bool, build: F) -> Result<Vec<T>> where F: Fn() -> Result<T> {
...
pub fn build_option<T, F>(confirm: &str, default: bool, build: F) -> Result<Option<T>> where F: Fn() -> Result<T> {
	let use_q = Question::confirm(confirm)
    	.default(default)
		.build();
