use std::{path::PathBuf, collections::BTreeMap, fs::File, io};
...
use memmap::Mmap;
use piz::{ZipArchive, read::{as_tree, DirectoryContents, FileTree, FileMetadata}};
...
impl<'a> Loader<'a> {
	...
	pub fn archive(mapping: &'a Option<Vec<u8>>) -> Result<Option<ZipArchive<'a>>> {
		let result = mapping.as_ref()
			.map(|m| ZipArchive::new(m))
			.invert()?;
		Ok(result)
	}

	pub fn tree(archive: &'a Option<ZipArchive<'a>>) -> Result<Option<DirectoryContents<'a>>> {
		let result = archive.as_ref()
			.map(|a| as_tree(a.entries()))
			.invert()?;
		Ok(result)
	}

	fn backend(target: &Utf8PathBuf, archive: &'a Option<ZipArchive<'a>>, tree: &'a Option<DirectoryContents<'a>>) -> Result<Backend<'a>> {
		use Backend::*;
		if target.is_dir() {
			return Ok(Folder);
		}
	...
	pub fn new(target: Utf8PathBuf, archive: &'a Option<ZipArchive<'a>>, tree: &'a Option<DirectoryContents<'a>>) -> Result<Self> {
	...
	fn get_path<P>(&self, path: P) -> Utf8PathBuf where P: AsRef<Utf8Path> {
		use Backend::*;
		match &self.backend {
	...
	pub fn parse<T>(content: String) -> Result<T> where T: DeserializeOwned {
	...
	fn archived_content_file<P>(file: &FileMetadata, kind: P) -> Option<KeyedPath> where P: AsRef<Utf8Path> {
	...
	pub fn map_content<P, T, F>(&self, path: P, mapper: F) -> Result<BTreeMap<String, T>> where P: AsRef<Utf8Path>, F: Fn(Utf8PathBuf) -> Result<T> {
		use Backend::*;
		match &self.backend {
			Folder => {
				let full = self.get_path(&path);
	...
	pub fn load_content<P, T>(&self, path: P) -> Result<Contents<T>> where P: AsRef<Utf8Path>, T: DeserializeOwned {
